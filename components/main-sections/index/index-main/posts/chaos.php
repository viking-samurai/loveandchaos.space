<article class="post">	
	<header>
		<div class="title">
			<h1><a href="index.php">On chaos, economics, and happiness</a></h1>
		</div>
	</header>

	<a href="single.html" class="image featured"><img src="images/Rebecca-Guay/chaos2.png" alt="" /></a>
	<p>Mauris neque quam, fermentum ut nisl vitae, convallis maximus nisl. Sed mattis nunc id lorem euismod placerat. 
		Vivamus porttitor magna enim, ac accumsan tortor cursus at. Phasellus sed ultricies mi non congue ullam corper.</p>
	
	<footer>
		<ul class="actions">
			<?php include 'components/main-sections/index/index-main/pagination/deeper-button.php' ?>
			<?php include 'components/main-sections/index/index-main/pagination/surface-button.php' ?>
		</ul>
		<ul class="stats">
			<li><a href="#">General</a></li>
			<li><a href="#" class="icon solid fa-heart">28</a></li>
			<li><a href="#" class="icon solid fa-comment">128</a></li>
		</ul>
	</footer>

</article>
